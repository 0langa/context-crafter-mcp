...
# Allowed paths for resource reading (populated by generation)
_ALLOWED_RESOURCE_PATHS: set[str] = set()
...
async def call_tool(name: str, arguments: dict) -> list:
    ...
    if name in ("generate_all", "generate_context"):
        ...
        # Register written files as safe resource paths
        for w in result.get("written", []):
            _ALLOWED_RESOURCE_PATHS.add(str(Path(w).resolve()))
        return [
            TextContent(
                type="text",
...
async def read_resource(uri: str) -> list[ReadResourceContents]:
    ...
    if resolved not in _ALLOWED_RESOURCE_PATHS:
        return [
