...
def _register_written_files(written: list[str]) -> None:
    ...
    for w in written:
        path = Path(w).resolve()
        if path.exists():
            uri = f"context-crafter://latest/{path.name}"
            _REGISTERED_RESOURCES[uri] = path
...
CAPABILITIES_TEXT = (
    ...
    "COMMANDS.md, VALIDATION_REPORT.md, SCAN_REPORT.md, "
    "EVIDENCE_LEDGER.json, CONTEXT_MANIFEST.json, RUN_STATE.json.\n"
...
async def call_tool(name: str, arguments: dict) -> list:
    ...
    if name == "generate_context":
        ...
        _register_written_files(result.get("written", []))
        return [
            TextContent(
                type="text",
    ...
    if name == "validate_generated_context":
        ...
        return [
            TextContent(
                type="text",
                text=json.dumps(val_result.to_dict(), indent=2),
            )
        ]
...
async def list_resources() -> list:
    """List currently registered generated resources."""
...
async def read_resource(uri: str) -> list[ReadResourceContents]:
    """Read a generated resource by context-crafter://latest/<filename> URI.

    Only allows reading files that were previously generated by this server session.
    """
    uri_str = str(uri)
    path = _REGISTERED_RESOURCES.get(uri_str)
    if path is None:
        return [
            ReadResourceContents(
                content=f"Access denied: {uri} is not a registered generated resource.",
