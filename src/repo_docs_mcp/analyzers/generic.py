...
from repo_docs_mcp.filesystem import (
...
def analyze_generic(
    ...
    for fi in safe_scan(path, max_depth=cfg.max_depth + 1, max_files_per_dir=cfg.max_files_per_dir, include_dirs=True):
        if fi.is_dir:
            name = fi.path.name.lower()
            if name in TEST_DIR_HINTS:
                test_dirs.add(fi.rel_path or fi.path.name)
            if name in SOURCE_DIR_HINTS:
                source_dirs.add(fi.rel_path or fi.path.name)
            if name in DOCS_HINTS:
                docs_dirs.add(fi.rel_path or fi.path.name)
            continue
