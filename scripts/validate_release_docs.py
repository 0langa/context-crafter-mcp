...
def validate() -> list[str]:
    ...
    release_tag_pattern = re.compile(r"Latest public git tag(?: on the remote)?: \*{0,2}`?1\.0\.0`?")
    if not release_tag_pattern.search(roadmap) and "Latest public git tag: `1.0.0`" not in project_state:
        issues.append("Release docs must record latest public git tag 1.0.0.")
